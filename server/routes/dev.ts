...
export const seedAndDraw: RequestHandler = async (_req, res) => {
  ...
  const eligible = await all<any>(
    `SELECT * FROM players WHERE paid = 1 ORDER BY RANDOM()`,
  );
  ...
  const result: { players: any[] }[] = Array.from(
    { length: teamCount },
    () => ({ players: [] }),
  );
  const pop = (pos: string, idx: number) => {
    const p = buckets[pos].shift();
    if (p) result[idx].players.push(p);
  };
  ...
  while (leftovers.length) {
    result[t % teamCount].players.push(leftovers.shift());
    t++;
  }
  ...
  for (let i = 0; i < ids.length; i += 2) {
    ...
    const b = ids[i + 1];
    if (a && b)
      await insert(`INSERT INTO matches (team_a_id, team_b_id) VALUES (?, ?)`, [
        a,
        b,
      ]);
  }
