...
export const seedAndDraw: RequestHandler = async (_req, res) => {
  ...
  if (countTeams === 0) {
    const t1 = { id: await getNextId("teams"), name: "Time 1", color: null, created_at: new Date().toISOString() };
    const t2 = { id: await getNextId("teams"), name: "Time 2", color: null, created_at: new Date().toISOString() };
    await teamsCol.insertMany([t1 as any, t2 as any]);
    const names = ["Ana", "Bruno", "Carlos", "Diana", "Edu", "Fabi", "Gabi", "Hugo", "Ivo", "Júlia", "Kiko", "Lia"];
  ...
  const teams = await teamsCol.find({}, { projection: { _id: 0, id: 1, name: 1 } }).toArray();
  ...
  for (let i = 0; i < ids.length; i += 2) {
    ...
    if (a && b) {
      ...
      await matchesCol.insertOne({ id, team_a_id: a, team_b_id: b, status: "scheduled", stage: "classificatoria", created_at: new Date().toISOString() });
    }
  }
